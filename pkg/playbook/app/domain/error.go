...
var (
	...
	ErrNoNGTSTokenURL = fmt.Errorf("no tokenURL defined in credentials. tokenURL is required to request an access token from Palo Alto Networks Next-Generation Trust Security (NGTS) when using client credentials grant type")
	// ErrNoNGTSClientId is thrown when platform is Palo Alto Networks Next-Generation Trust Security (NGTS) and no config.credentials.clientId is defined
	ErrNoNGTSClientId = fmt.Errorf("no clientId defined. Palo Alto Networks Next-Generation Trust Security (NGTS) platform requires a clientId to request OAuth2 token")
	// ErrNoNGTSClientSecret is thrown when platform is Palo Alto Networks Next-Generation Trust Security (NGTS) and no config.credentials.clientSecret is defined
	ErrNoNGTSClientSecret = fmt.Errorf("no clientSecret defined. Palo Alto Networks Next-Generation Trust Security (NGTS) platform requires a clientSecret to request OAuth2 token")
	// ErrNoNGTSScope is thrown when platform is Palo Alto Networks Next-Generation Trust Security (NGTS) and no config.credentials.scope is defined
	ErrNoNGTSScope = fmt.Errorf("no scope defined. Palo Alto Networks Next-Generation Trust Security (NGTS) platform requires a scope to request OAuth2 token")
	// ErrNoNGTSAccessToken is thrown when platform is Palo Alto Networks Next-Generation Trust Security (NGTS) and no config.credentials.accessToken is defined
	ErrNoNGTSAccessToken = fmt.Errorf("no accessToken defined. Palo Alto Networks Next-Generation Trust Security (NGTS) platform requires an OAuth2 access token for authentication")
	...
	ErrAmbiguousNGTSCreds = fmt.Errorf("unable to disambiguate multiple  Palo Alto Networks Next-Generation Trust Security (NGTS) credentials. Only ONE of accessToken or client credentials should be defined")
)
