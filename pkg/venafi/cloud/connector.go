...
func (c *Connector) ProvisionCertificate(req *endpoint.ProvisioningRequest, options *endpoint.ProvisioningOptions) (provisioningMetadata endpoint.ProvisioningMetadata, err error) {
	...
	_, err = cloudkeystores.ProvisionCertificate(ctx, graphqlClient, certificateIdString, keystoreIdString, wsClientIdString, provisioningOptions)
	if err != nil {
		return nil, err
	}
...
func validateProvisioningInputs(req endpoint.ProvisioningRequest) error {
	if req.KeystoreId != nil && (req.KeystoreName != nil || req.ProviderName != nil) {
		return fmt.Errorf("only should keystoreId OR keystoreName and providerName can be provided")
	}
	if req.KeystoreId == nil {
		if req.KeystoreName == nil {
