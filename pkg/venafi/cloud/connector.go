...
func (c *Connector) ProvisionCertificate(certificateId string, providerName *string, keystoreName *string, keystoreId *string, options *endpoint.ProvisioningOptions) (provisioningMetadata endpoint.ProvisioningMetadata, err error) {
	log.Printf("Starting Provisioning Flow for Certificate with ID: %s", certificateId)

	log.Println("Validating inputs")
	err = validateProvisioningInputs(keystoreId, keystoreName, providerName)
	if err != nil {
		return nil, err
	}
	log.Println("Inputs are valid")

	// Is certificate generated by VCP?
	log.Printf("Validating if certificate as generated by VCP")
	...
	if keystoreId == nil && (keystoreName != nil && providerName != nil) {
		...
		if keystoreId == nil {
			return nil, fmt.Errorf("could not find Keystore ID for Keystore Name %s and Cloud Provider Name %s", *keystoreName, *providerName)
		}
	}
	keystoreIdString := *keystoreId
	...
	data, err := cloudkeystores.GetCloudKeystoresByKeystoreId(ctx, graphqlClient, keystoreId)
	if err != nil {
		return nil, err
	}
	...
	wsClientIdString := wsClientId.String()

	wsConn, err := websocket.Subscribe(c.apiKey, c.baseURL, wsClientIdString)
	if err != nil {
		return nil, err
	}

	log.Printf("Provisioning Certificate ID %s for Keystore %s", certificateId, keystoreIdString)
	_, err = cloudkeystores.ProvisionCertificate(ctx, graphqlClient, certificateId, keystoreIdString, wsClientIdString, provisioningOptions)
	if err != nil {
		return nil, err
	}
	...
	log.Printf("Getting Cloud Metadata of Certificate ID %s and Keystore ID: %s", certificateId, keystoreIdString)
	respMap := ar.Data.Result
	cloudMetadata, err := getCloudMetadataFromWebsocketResponse(respMap, cloudKeystore.Type, keystoreIdString)
	if err != nil {
		return nil, err
	}
	log.Printf("Successfully got Cloud Metadata for Certificate ID %s and Keystore ID: %s", certificateId, keystoreIdString)

	log.Printf("Successfully finished Provisioning Flow for Certificate ID %s and Keystore ID %s", certificateId, keystoreIdString)
	return cloudMetadata, nil
}

func (c *Connector) getGraphqlClient() graphql.Client {
	graphqlURL := fmt.Sprintf("%sgraphql", c.baseURL)

	// We provide every auth here
...
func validateProvisioningInputs(keystoreId *string, keystoreName *string, providerName *string) error {
	if keystoreId != nil && (keystoreName != nil || providerName != nil) {
		return fmt.Errorf("only should keystoreId OR keystoreName and providerName can be provided")
	}
	if keystoreId == nil {
		if keystoreName == nil {
