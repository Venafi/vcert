...
func parseDEKInfo(httpStatusCode int, httpStatus string, body []byte) (*EdgeEncryptionKey, error) {
	...
	default:
		if body != nil {
			...
			if err == nil {
				respError := fmt.Sprintf("unexpected status code on Palo Alto Networks Next-Gen Trust Security (NGTS) retrieving DEK's info. Status: %s\n", httpStatus)
		...
		return nil, fmt.Errorf("unexpected status code on Palo Alto Networks Next-Gen Trust Security (NGTS) retrieving DEK's info. Status. Status: %s", httpStatus)
	}
}
