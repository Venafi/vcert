...
func (c Certificate) ToCertificateInfo() certificate.CertificateInfo {
	...
	return certificate.CertificateInfo{
		...
		SANS: certificate.Sans{
			...
			// currently not supported on Palo Alto Networks Next-Gen Trust Security (NGTS)
...
func ParseCertificateSearchResponse(httpStatusCode int, body []byte) (searchResult *CertificateSearchResponse, err error) {
	...
	default:
		...
		return nil, fmt.Errorf("unexpected status code on Palo Alto Networks Next-Gen Trust Security (NGTS) certificate search. Status: %d", httpStatusCode)
	}
}
