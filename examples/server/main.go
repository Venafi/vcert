...
func main() {
	...
	server := &http.Server{
		Addr:         ":8080",
		Handler:      mux,
		ReadTimeout:  5 * time.Second,   // Prevent slow clients from taking too long to send requests
		WriteTimeout: 10 * time.Second,  // Prevent slow responses from blocking connections
		IdleTimeout:  120 * time.Second, // Limit idle connections
	}
