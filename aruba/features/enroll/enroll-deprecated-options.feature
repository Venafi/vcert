...
# VCert enroll -tpp-url %TPPdeprecated_URL% -tpp-user %TPP_USER% -tpp-password %TPP_PASS% -z "%ECC_POLICY%" -csr service -key-type ecdsa -key-curve p521 -cn service-gen-ecc.vcert.example -format json -key-password %KEY_PASS%
# if ERRORLEVEL 1 goto :DONE
# timeout /t 10
...
#  timeout /t 10
  Scenario: ~ Service Generated CSR with SANS and should be no log output ~
    When I enroll random certificate using TPPdeprecated with -csr service -san-dns one.vcert.example -san-dns two.vcert.example -san-ip 10.20.30.40 -san-ip 198.168.144.120 -san-email zack.jackson@vcert.example -format json -key-password newPassw0rd!
...
# timeout /t 10
  Scenario: ~ User Provided CSR with SANs ~
    ...
    And I enroll certificate using TPPdeprecated with -csr file:user-provided-with-sans.req -cert-file c.pem
    And I decode certificate from file "c.pem"
    And that certificate should contain "DNS:one.vcert.example"
    And that certificate should contain "DNS:two.vcert.example"
    And that certificate should contain "email:zack.jackson@vcert.example"
...
# timeout /t 10
  Scenario: ~ User Provided CSR with full Subject DN ~
    ...
    And I enroll certificate using TPPdeprecated with -csr file:user-provided-full-subject.req -format json
    And I get JSON response
